...
def build_stmap_bbox():
    ...
    grp.addKnob(nuke.Text_Knob('fmt_help', '',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Source = the resolution the STMap\'s UVs are normalized against '
        '(usually the CG plate). Target = where the distorted output lives. '
        'When the STMap rescales (e.g. CG 4K &rarr; delivery 2K), set these '
        'differently.</span>'))
    ...
    grp.addKnob(nuke.Text_Knob('trim_help', '',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Pixels to ignore at each edge of the STMap before<br>'
        'sampling. Use to skip ringing or black bleed at edges.</span>'))
    ...
    grp.addKnob(nuke.Text_Knob('sec_live', '<b>Live values</b>',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Updated by Compute and live_update.<br>'
        'Never animated.</span>'))
    ...
    grp.addKnob(nuke.Text_Knob('sec_baked', '<b>Baked values</b>',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Drive the internal Crop. Animated by Bake.<br>'
        'Scrub to compare with Live above &mdash; if they<br>'
        'differ at a frame, that key is wrong.</span>'))
    ...
    grp.addKnob(nuke.Text_Knob('out_hint', '',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Baked values mirror onto the internal Crop.<br>'
        'Compute switches Crop\'s intersect off so the<br>'
        'bbox can grow past the input.</span>'))
    ...
    kextract.setTooltip(
        ...
        'box. The new Crop is connected to the same input as STMapBBox.img '
        'and is independent afterwards -- you can delete STMapBBox or '
        'detach it.\n\n'
    ...
    grp.addKnob(nuke.Text_Knob('anim_hint', '',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Bake = static keyframes (cheap playback).<br>'
        'Live = recompute on each frame change<br>'
        '(always current, slower).</span>'))
    ...
    notes = (
        ...
        'Predicts the bbox of the distorted CG: for each<br>'
        'pixel on a grid across the STMap, samples (U, V),<br>'
        'checks whether (U&middot;src_w, V&middot;src_h) lands inside the<br>'
        '<b>img</b> input\'s bbox X. The set of output pixels that<br>'
        'pass this test is the <i>active region</i> Y &mdash; where<br>'
        'the distorted CG will have content. Y lives in the<br>'
        '<i>target</i> plate\'s pixel space and is written to the bbox<br>'
        'knobs and the internal Crop.<br><br>'
        '<b>Inputs</b><br>'
        '&nbsp;&nbsp;<i>img</i> (input 0) &mdash; the CG render whose bbox<br>'
        '&nbsp;&nbsp;is being updated. Its current bbox X drives the test.<br>'
        '&nbsp;&nbsp;<i>stmap</i> (input 1) &mdash; the STMap that will<br>'
        '&nbsp;&nbsp;be applied downstream.<br><br>'
        '<b>Resolution change</b> &mdash; if your STMap retargets<br>'
        '(e.g. CG at 4K &rarr; delivery at 2K, or anamorphic<br>'
        'squeeze/stretch), the source and target formats differ.<br>'
        'Default <i>auto</i> mode reads source from <b>img</b> and<br>'
        'target from <b>stmap</b>, which works as long as both<br>'
        'inputs sit at their natural resolution. For unusual setups<br>'
        '(manually reformatted upstream, target format differs from<br>'
        'STMap container, etc.) flip to <i>manual</i> and set the<br>'
        'four numbers; <i>Pick Up Formats</i> seeds them from the<br>'
        'current inputs.<br><br>'
        '<b>Y can grow past X</b> &mdash; that\'s the whole<br>'
        'point. The internal Crop is configured with <i>intersect=False</i><br>'
        'on Compute so the new bbox isn\'t clipped to the<br>'
        'original. Pixels in Y but outside X get the default<br>'
        'Crop fill (black) since there is no original CG data<br>'
        'there.<br><br>'
        '<b>Channels</b><br>'
        'Default reads rgba.red / rgba.green. If your STMap stores<br>'
        'UVs in forward.u / forward.v (or backward.*), change the<br>'
        'channel knobs to point at those instead. Test Inputs samples<br>'
        'all three pairs so you can see which one has<br>'
        'the real STMap data.<br><br>'
        '<b>Grid resolution</b><br>'
        'The boundary of Y is uncertain to within one grid<br>'
        'cell, so the tool adds <i>max(step_x, step_y)</i> to the<br>'
        'safety margin automatically. Higher resolution &rarr; tighter bbox<br>'
        '&rarr; slower compute. 96 is fine for most lens distortion;<br>'
        'bump to 256 for tight optimisation passes. <i>bake grid</i><br>'
        'is a separate, lower default (48) used only during Bake<br>'
        'to keep it fast.<br><br>'
        '<b>Animation</b><br>'
        '&nbsp;&nbsp;<i>Bake</i> &mdash; runs Compute at every frame in<br>'
        '&nbsp;&nbsp;[first, last] step N and writes keyframes onto the<br>'
        '&nbsp;&nbsp;bbox knobs and the internal Crop. Existing keys are<br>'
        '&nbsp;&nbsp;cleared first. Drives the same UI cycle as live_update<br>'
        '&nbsp;&nbsp;so values match. Use once the shot is locked.<br>'
        '&nbsp;&nbsp;<i>Live update</i> &mdash; recomputes on every frame change.<br>'
        '&nbsp;&nbsp;Throttled to one compute per frame. Useful while iterating<br>'
        '&nbsp;&nbsp;or scrubbing; switch off (or bake) before playback.<br>'
        '&nbsp;&nbsp;<i>Extract External Crop</i> &mdash; copies the current bbox<br>'
        '&nbsp;&nbsp;values and animation curves to a new standalone Crop<br>'
        '&nbsp;&nbsp;node in the parent graph. The new node is<br>'
        '&nbsp;&nbsp;disconnected so you can wire it wherever you need.</p>'
    )
