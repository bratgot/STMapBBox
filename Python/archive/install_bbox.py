...
BAKE_CODE = (
    ...
    "                # restore both speedup overrides\n"
    ...
    "            nuke.frame(f)\n"
    "            # SPEEDUP 3: 10ms delay (was 50). Smallest interval that\n"
    "            # still lets Nuke complete a UI cycle between steps; goes\n"
    "            # below ~5ms and updateUI starts to drop frames.\n"
...
def build_stmap_bbox():
    ...
    grp.addKnob(nuke.Text_Knob('sec_baked', '<b>Baked values</b>',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Drive the internal Crop. Animated by Bake. Scrub to compare with '
    ...
    grp.addKnob(nuke.Text_Knob('out_hint', '',
        '<span style="color:#888; font-size:10px; font-style:italic;">'
        'Baked values mirror onto the internal Crop. Compute switches Crop\'s '
    ...
    notes = (
        ...
        'across the STMap, samples (U, V), checks whether '
        '(U&middot;src_w, V&middot;src_h) lands inside the <b>img</b> input\'s '
        'bbox X. The set of output pixels that pass this test is the '
        '<i>active region</i> Y &mdash; where the distorted CG will have '
        'content. Y lives in the <i>target</i> plate\'s pixel space and is '
        'written to the bbox knobs and the internal Crop.<br><br>'
        '<b>Inputs</b><br>'
        ...
        '<b>Resolution change</b> &mdash; if your STMap retargets '
        '(e.g. CG at 4K &rarr; delivery at 2K, or anamorphic squeeze/stretch), '
        'the source and target formats differ. Default <i>auto</i> mode '
        'reads source from <b>img</b> and target from <b>stmap</b>, which '
        'works as long as both inputs sit at their natural resolution. '
        'For unusual setups (manually reformatted upstream, target format '
        'differs from STMap container, etc.) flip to <i>manual</i> and set '
        'the four numbers; <i>Pick Up Formats</i> seeds them from the '
